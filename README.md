项目基于[PKUAutoElective](https://github.com/zhongxinghong/PKUAutoElective)。

## 补丁说明

由于本地部署模型对服务器算力有一定要求，且由于选课网不时更新其验证码生成器，若重新训练模型则需提前在补退选一阶段爬取验证码图集，相当耗时费力。
故本项目将原本利用本地模型识别验证码的机制更改为利用商用验证码识别平台[云码平台](https://www.jfbym.com/)在线识别验证码。使用前需要注册云码平台并将API token写入config.ini，同时使用时**需要网络连接**。
其它未作更改。单独运行recognizerAPI.py可检测识图可用性。

由于更换了专业的打码平台，本项目的识别准确度和识别速度都较原项目有较大提升，基本可以做到即刷即抢。但使用时需注意可能存在的bug。

安装和使用流程基本与原项目相同，但不再依赖opencv。项目也提供了更新过的Dockerfile。
